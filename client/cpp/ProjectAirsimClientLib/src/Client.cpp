...
void PackJson(msgpack::packer<msgpack::sbuffer>* ppacker,
              const nlohmann::json& value) {
  if (value.is_null()) {
    ppacker->pack_nil();
  } else if (value.is_boolean()) {
    ppacker->pack(value.get<bool>());
  } else if (value.is_number_integer()) {
    ppacker->pack(value.get<std::int64_t>());
  } else if (value.is_number_unsigned()) {
    ppacker->pack(value.get<std::uint64_t>());
  } else if (value.is_number_float()) {
    ppacker->pack(value.get<double>());
  } else if (value.is_string()) {
    ppacker->pack(value.get<std::string>());
  } else if (value.is_array()) {
    ppacker->pack_array(value.size());
    for (const auto& item : value) PackJson(ppacker, item);
  } else if (value.is_object()) {
    ppacker->pack_map(value.size());
    for (auto it = value.begin(); it != value.end(); ++it) {
      ppacker->pack(it.key());
      PackJson(ppacker, it.value());
    }
  } else {
    ppacker->pack_nil();
  }
...
class Client::Impl {
 ...
 public:
  ...
  Status SetInteractiveFeature(const std::string& str_feature_name,
                               bool f_enable, bool* pf_enabled_out);
  Status GetBuildCommitHash(std::string* pstr_hash_out);
  ...
  Status Publish(const char* sz_topic_name, const json& json_message);
...
ASC_DECL Status Client::SetInteractiveFeature(const std::string& str_feature_name,
                                              bool f_enable,
                                              bool* pf_enabled_out) {
  return (pclient_impl_->SetInteractiveFeature(str_feature_name, f_enable,
                                               pf_enabled_out));
}

ASC_DECL Status Client::GetBuildCommitHash(std::string* pstr_hash_out) {
  return (pclient_impl_->GetBuildCommitHash(pstr_hash_out));
}
...
ASC_DECL Status Client::Publish(const char* sz_topic_name,
                                const nlohmann::json& json_message) {
  return (pclient_impl_->Publish(sz_topic_name, json_message));
}
...
Status Client::Impl::SetInteractiveFeature(const std::string& str_feature_name,
                                           bool f_enable,
                                           bool* pf_enabled_out) {
  Status status;
  Message message_response;
  json json_params = {
      {"feature_id", str_feature_name},
      {"enable", f_enable},
  };

  if ((status = Request("/Sim/SetInteractiveFeature", json_params,
                        &message_response)) == Status::OK) {
    ResponseMessage response_message;

    response_message.Deserialize(message_response);
    if (response_message.GetErrorCode() == 0)
      *pf_enabled_out = response_message.GetResult().is_boolean() &&
                        static_cast<bool>(response_message.GetResult());
    else {
      status = Status::RejectedByServer;

      log.ErrorF(
          "Server method \"/Sim/SetInteractiveFeature\" failed: error %d: %s",
...
Status Client::Impl::GetBuildCommitHash(std::string* pstr_hash_out) {
  Status status;
  Message message_response;

  if ((status = Request("/Sim/GetBuildCommitHash", json(),
                        &message_response)) == Status::OK) {
    ResponseMessage response_message;

    response_message.Deserialize(message_response);
    if (response_message.GetErrorCode() == 0)
      *pstr_hash_out = static_cast<std::string>(response_message.GetResult());
    else {
      status = Status::RejectedByServer;

      log.ErrorF(
          "Server method \"/Sim/GetBuildCommitHash\" failed: error %d: %s",
          response_message.GetErrorCode(),
          static_cast<std::string>(response_message.GetResult()["message"])
              .c_str());
    }
...
Status Client::Impl::Publish(const char* sz_topic_name,
                             const json& json_message) {
  if (!is_connected_) return (Status::NotConnected);
  if (sz_topic_name == nullptr) return (Status::Failed);

  try {
    msgpack::sbuffer body_buffer;
    msgpack::sbuffer frame_buffer;
    const int kMessageFrameType = 2;
    msgpack::packer<msgpack::sbuffer> body_packer(body_buffer);

    PackJson(&body_packer, json_message);
    std::tuple<int, std::string, std::string> frame(
        kMessageFrameType, sz_topic_name,
        std::string(body_buffer.data(), body_buffer.size()));
    msgpack::pack(frame_buffer, frame);

    return (pnng_server_channel_topics_->Send(
        reinterpret_cast<const uint8_t*>(frame_buffer.data()),
        frame_buffer.size()));
  } catch (...) {
