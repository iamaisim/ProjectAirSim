...
inline void from_json(const nlohmann::json& j, ReadyState& ready_state) {
  ready_state.is_ready = j.at("ready_val");
  ready_state.message = j.at("ready_message");
}

inline void from_json(const nlohmann::json& j, GeoPosition& geo_position) {
  geo_position.latitude = j.at("latitude");
  geo_position.longitude = j.at("longitude");
  geo_position.altitude = j.at("altitude");
}
...
inline void to_json(nlohmann::json& j, const GeoPosition& geo_position) {
  j["latitude"] = geo_position.latitude;
  j["longitude"] = geo_position.longitude;
  j["altitude"] = geo_position.altitude;
}
