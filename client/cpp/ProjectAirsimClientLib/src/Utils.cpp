...
void UpdateClientVersion(HMODULE hmodule) {
  ...
  if ((GetModuleFileNameW(hmodule, wzPathModule, _countof(wzPathModule)) == 0) ||
      ((cbVersionInfo = GetFileVersionInfoSizeW(wzPathModule, nullptr)) == 0)) {
    return;
  }

  // Get the product version string
  vecVersionInfo.resize(cbVersionInfo);
  if ((GetFileVersionInfoW(wzPathModule, 0, (DWORD)cbVersionInfo,
                           vecVersionInfo.data()) == 0) ||
      (VerQueryValueW(vecVersionInfo.data(),
...
#ifndef _WIN32
void UpdateClientVersion(void* /*hmodule*/) {}
#endif
