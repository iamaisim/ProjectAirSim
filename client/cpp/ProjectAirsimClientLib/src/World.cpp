...
class World::Impl {
 ...
 public:
  Status CreateVoxelGrid(const pasc::Pose& position, int x_size, int y_size,
  ...
  Status ImportNEDTrajectory(const std::string& traj_name,
                             const std::vector<float>& time,
                             const std::vector<float>& pose_x,
                             const std::vector<float>& pose_y,
                             const std::vector<float>& pose_z,
                             const std::vector<float>& pose_roll,
                             const std::vector<float>& pose_pitch,
                             const std::vector<float>& pose_yaw,
                             const std::vector<float>& vel_lin_x,
                             const std::vector<float>& vel_lin_y,
                             const std::vector<float>& vel_lin_z);
  ...
  Status RequestWorldJson(const std::string& method,
                          const json& json_params,
                          json* pjson_result_out) const;
  Status RequestWorldBool(const std::string& method,
                          const json& json_params,
                          bool* pf_result_out) const;
...
ASC_DECL Status
World::CreateVoxelGrid(const pasc::Pose& position, int x_size, int y_size,
...
ASC_DECL Status World::GetSimClockType(std::string* pstr_clock_type_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson("GetSimClockType", json::object(),
                                               &result));
  if (pstr_clock_type_out != nullptr) *pstr_clock_type_out = result.get<std::string>();
  return (Status::OK);
}

ASC_DECL Status World::GetSimTime(std::int64_t* ptime_nanos_out) const {
  json result;
  RETURN_CATCH_STATUS(
      pimpl_->RequestWorldJson("GetSimTime", json::object(), &result));
  if (ptime_nanos_out != nullptr) *ptime_nanos_out = result.get<std::int64_t>();
  return (Status::OK);
}

ASC_DECL Status World::Pause(std::string* pstr_state_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "Pause", json{{"do_pause", true}}, &result));
  if (pstr_state_out != nullptr) *pstr_state_out = result.get<std::string>();
  return (Status::OK);
}

ASC_DECL Status World::Resume(std::string* pstr_state_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "Pause", json{{"do_pause", false}}, &result));
  if (pstr_state_out != nullptr) *pstr_state_out = result.get<std::string>();
  return (Status::OK);
}

ASC_DECL Status World::IsPaused(bool* pf_is_paused_out) const {
  RETURN_CATCH_STATUS(
      pimpl_->RequestWorldBool("IsPaused", json::object(), pf_is_paused_out));
}

ASC_DECL Status World::ContinueForSimTime(std::int64_t delta_time_nanos,
                                          bool wait_until_complete,
                                          std::int64_t* ptime_nanos_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ContinueForSimTime",
      json{{"delta_time", delta_time_nanos},
...
ASC_DECL Status World::ContinueUntilSimTime(std::int64_t target_time_nanos,
                                            bool wait_until_complete,
                                            std::int64_t* ptime_nanos_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ContinueUntilSimTime",
      json{{"target_time", target_time_nanos},
           {"wait_until_complete", wait_until_complete}},
      &result));
  if (ptime_nanos_out != nullptr) *ptime_nanos_out = result.get<std::int64_t>();
  return (Status::OK);
...
ASC_DECL Status World::ContinueForNSteps(int n_steps, bool wait_until_complete,
                                         std::int64_t* ptime_nanos_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ContinueForNSteps",
      json{{"n_steps", n_steps}, {"wait_until_complete", wait_until_complete}},
      &result));
  if (ptime_nanos_out != nullptr) *ptime_nanos_out = result.get<std::int64_t>();
  return (Status::OK);
}

ASC_DECL Status World::ContinueForSingleStep(bool wait_until_complete,
                                             std::int64_t* ptime_nanos_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ContinueForSingleStep",
      json{{"wait_until_complete", wait_until_complete}}, &result));
  if (ptime_nanos_out != nullptr) *ptime_nanos_out = result.get<std::int64_t>();
  return (Status::OK);
}

ASC_DECL Status World::SetSunlightIntensity(float intensity,
                                            bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetSunLightIntensity", json{{"intensity", intensity}}, pf_success_out));
}

ASC_DECL Status World::GetSunlightIntensity(float* pintensity_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson("GetSunLightIntensity",
                                               json::object(), &result));
  if (pintensity_out != nullptr) *pintensity_out = result.get<float>();
  return (Status::OK);
}

ASC_DECL Status World::SetCloudShadowStrength(float strength,
                                              bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetCloudShadowStrength", json{{"strength", strength}}, pf_success_out));
}

ASC_DECL Status World::GetCloudShadowStrength(float* pstrength_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson("GetCloudShadowStrength",
                                               json::object(), &result));
  if (pstrength_out != nullptr) *pstrength_out = result.get<float>();
  return (Status::OK);
}

ASC_DECL Status World::SetWindVelocity(float v_x, float v_y, float v_z,
                                       bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetWindVelocity", json{{"v_x", v_x}, {"v_y", v_y}, {"v_z", v_z}},
      pf_success_out));
}

ASC_DECL Status World::GetWindVelocity(Vector3* pwind_velocity_out) const {
  json result;
  RETURN_CATCH_STATUS(
      pimpl_->RequestWorldJson("GetWindVelocity", json::object(), &result));
  if (pwind_velocity_out != nullptr && result.is_array() && result.size() >= 3) {
    pwind_velocity_out->x = result[0].get<float>();
    pwind_velocity_out->y = result[1].get<float>();
    pwind_velocity_out->z = result[2].get<float>();
  }
  return (Status::OK);
}

ASC_DECL Status World::ListActors(VecStr* pvecstr_actor_names_out) const {
  json result;
  RETURN_CATCH_STATUS(
      pimpl_->RequestWorldJson("ListActors", json::object(), &result));
  if (pvecstr_actor_names_out != nullptr) {
    *pvecstr_actor_names_out = result.get<VecStr>();
  }
  return (Status::OK);
}

ASC_DECL Status World::ListObjects(const std::string& name_regex,
                                   VecStr* pvecstr_object_names_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ListObjects", json{{"name", name_regex}}, &result));
  if (pvecstr_object_names_out != nullptr) {
    *pvecstr_object_names_out = result.get<VecStr>();
  }
  return (Status::OK);
}

ASC_DECL Status World::ListAssets(const std::string& name_regex,
                                  VecStr* pvecstr_asset_names_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "ListAssets", json{{"name", name_regex}}, &result));
  if (pvecstr_asset_names_out != nullptr) {
    *pvecstr_asset_names_out = result.get<VecStr>();
  }
  return (Status::OK);
}

ASC_DECL Status World::GetObjectPose(const std::string& object_name,
                                     json* pjson_pose_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetObjectPose", json{{"object_name", object_name}}, pjson_pose_out));
}

ASC_DECL Status World::GetObjectPoses(const VecStr& object_names,
                                      json* pjson_poses_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetObjectPoses", json{{"object_names", object_names}}, pjson_poses_out));
}

ASC_DECL Status World::SetObjectPose(const std::string& object_name,
                                     const json& json_pose, bool teleport,
                                     bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectPose",
      json{{"object_name", object_name}, {"pose", json_pose}, {"teleport", teleport}},
      pf_success_out));
}

ASC_DECL Status World::GetObjectScale(
    const std::string& object_name, std::vector<float>* pvec_scale_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetObjectScale", json{{"object_name", object_name}}, &result));
  if (pvec_scale_out != nullptr) *pvec_scale_out = result.get<std::vector<float>>();
  return (Status::OK);
}

ASC_DECL Status World::SetObjectScale(const std::string& object_name,
                                      const std::vector<float>& scale,
                                      bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectScale", json{{"object_name", object_name}, {"scale", scale}},
      pf_success_out));
}

ASC_DECL Status World::SpawnObject(const std::string& object_name,
                                   const std::string& asset_path,
                                   const json& json_pose,
                                   const std::vector<float>& scale,
                                   bool enable_physics,
                                   std::string* pstr_spawned_name_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "SpawnObject",
      ...
      &result));
  if (pstr_spawned_name_out != nullptr) *pstr_spawned_name_out = result.get<std::string>();
  return (Status::OK);
}

ASC_DECL Status World::SpawnObjectFromFile(
...
ASC_DECL Status World::SpawnObjectAtGeo(
    ...
    bool enable_physics, std::string* pstr_spawned_name_out) {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "spawnObjectAtGeo",
      json{{"object_name", object_name},
...
ASC_DECL Status World::SpawnObjectFromFileAtGeo(
    const std::string& object_name, const std::string& file_format,
...
ASC_DECL Status World::DestroyObject(const std::string& object_name,
                                     bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "DestroyObject", json{{"object_name", object_name}}, pf_success_out));
}

ASC_DECL Status World::DestroyAllSpawnedObjects(bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "DestroyAllSpawnedObjects", json::object(), pf_success_out));
}

ASC_DECL Status World::EnableWeatherVisualEffects(bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SimSetWeatherVisualEffectsStatus", json{{"status", true}}, pf_success_out));
}

ASC_DECL Status World::DisableWeatherVisualEffects(bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SimSetWeatherVisualEffectsStatus", json{{"status", false}}, pf_success_out));
}

ASC_DECL Status World::ResetWeatherEffects(bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "ResetWeatherEffects", json::object(), pf_success_out));
}

ASC_DECL Status World::SetWeatherVisualEffectsParam(int param, float value,
                                                    bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetWeatherVisualEffectsParameter",
      json{{"param", param}, {"value", value}}, pf_success_out));
}

ASC_DECL Status World::GetWeatherVisualEffectsParam(
    json* pjson_params_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetWeatherVisualEffectsParameter", json::object(), pjson_params_out));
}

ASC_DECL Status World::SetObjectMaterial(
    const std::string& object_name, const std::string& material_asset_path,
    bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectMaterial",
      json{{"object_name", object_name},
...
ASC_DECL Status World::SetObjectTextureFromUrl(const std::string& object_name,
                                               const std::string& url,
                                               bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectTextureFromUrl",
      json{{"object_name", object_name}, {"url", url}}, pf_success_out));
}

ASC_DECL Status World::SetObjectTextureFromFile(
    const std::string& object_name, const std::string& texture_file_path,
    bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectTextureFromFile",
      json{{"object_name", object_name}, {"texture_file_path", texture_file_path}},
      pf_success_out));
}

ASC_DECL Status World::SetObjectTextureFromPackagedAsset(
    const std::string& object_name, const std::string& texture_asset_path,
    bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetObjectTextureFromPackagedAsset",
      json{{"object_name", object_name}, {"texture_asset_path", texture_asset_path}},
      pf_success_out));
}

ASC_DECL Status World::SwapObjectTexture(const std::string& tag, int tex_id,
                                         bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SwapObjectTexture", json{{"tag", tag}, {"tex_id", tex_id}},
      pf_success_out));
}

ASC_DECL Status World::SetLightObjectIntensity(
    const std::string& object_name, float new_intensity, bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetLightObjectIntensity",
      json{{"object_name", object_name}, {"new_intensity", new_intensity}},
      pf_success_out));
}

ASC_DECL Status World::SetLightObjectColor(
    const std::string& object_name, const std::vector<float>& color_rgb,
    bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetLightObjectColor",
      json{{"object_name", object_name}, {"color_rgb", color_rgb}},
      pf_success_out));
}

ASC_DECL Status World::SetLightObjectRadius(const std::string& object_name,
                                            float new_radius,
                                            bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetLightObjectRadius",
      json{{"object_name", object_name}, {"new_radius", new_radius}},
      pf_success_out));
}

ASC_DECL Status World::SetTimeOfDay(bool enabled, const std::string& datetime,
...
ASC_DECL Status World::GetTimeOfDay(json* pjson_time_of_day_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson("GetTimeOfDay", json::object(),
                                               pjson_time_of_day_out));
}

ASC_DECL Status World::SetSunPositionFromDateTime(
    const std::string& date_time, bool is_dst, bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetSunPositionFromDateTime",
      json{{"date_time", date_time}, {"is_dst", is_dst}}, pf_success_out));
}

ASC_DECL Status World::SwitchStreamingView(bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SwitchStreamingView", json::object(), pf_success_out));
}

ASC_DECL Status World::SetSegmentationIDByName(
    const std::string& mesh_name, int segmentation_id, bool is_name_regex,
    bool use_owner_name, bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetSegmentationIDByName",
      json{{"mesh_name", mesh_name},
...
ASC_DECL Status World::GetSegmentationIDByName(
    const std::string& mesh_name, bool use_owner_name,
    int* psegmentation_id_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetSegmentationIDByName",
      json{{"mesh_name", mesh_name}, {"use_owner_name", use_owner_name}},
      &result));
  if (psegmentation_id_out != nullptr) *psegmentation_id_out = result.get<int>();
  return (Status::OK);
}

ASC_DECL Status World::GetSegmentationIDMap(
    json* pjson_segmentation_map_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson("GetSegmentationIDMap",
                                               json::object(),
                                               pjson_segmentation_map_out));
}
...
ASC_DECL Status World::ImportNEDTrajectory(
    ...
    const std::vector<float>& vel_lin_x, const std::vector<float>& vel_lin_y,
    const std::vector<float>& vel_lin_z) {
  RETURN_CATCH_STATUS(pimpl_->ImportNEDTrajectory(
      traj_name, time, pose_x, pose_y, pose_z, pose_roll, pose_pitch, pose_yaw,
...
ASC_DECL Status World::PlotDebugTransformsWithNames(
...
ASC_DECL Status World::SetTraceLine(const ColorRGBA& color_rgba,
                                    float thickness, bool* pf_success_out) {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldBool(
      "SetTraceLine", json{{"color_rgba", color_rgba}, {"thickness", thickness}},
      pf_success_out));
}

ASC_DECL Status World::ToggleTrace(bool* pf_success_out) {
  RETURN_CATCH_STATUS(
      pimpl_->RequestWorldBool("ToggleTrace", json::object(), pf_success_out));
}

ASC_DECL Status World::Get3DBoundingBox(
    const std::string& object_name, int box_alignment,
    json* pjson_bounding_box_out) const {
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "Get3DBoundingBox",
      json{{"object_name", object_name}, {"box_alignment", box_alignment}},
      pjson_bounding_box_out));
}

ASC_DECL Status World::ImportGeoTrajectory(
    const std::string& traj_name, const std::vector<float>& time,
    const std::vector<float>& latitudes, const std::vector<float>& longitudes,
    const std::vector<float>& altitudes, const std::vector<float>& roll,
...
ASC_DECL Status World::GetSurfaceElevationAtPoint(float x, float y,
                                                  float* pelevation_out) const {
  json result;
  RETURN_CATCH_STATUS(pimpl_->RequestWorldJson(
      "GetSurfaceElevationAtPoint", json{{"x", x}, {"y", y}}, &result));
  if (pelevation_out != nullptr) *pelevation_out = result.get<float>();
  return (Status::OK);
}
...
Status World::Impl::RequestWorldJson(const std::string& method,
                                     const json& json_params,
                                     json* pjson_result_out) const {
  Message message_response;
  Status status;

  if ((status = pclient_->Request(str_parent_topic_ + "/" + method,
    ...
    if (response_message.GetErrorCode() != 0) {
      json json_result = response_message.GetResult();

      if (json_result.contains("message"))
        log.ErrorF("Server method \"%s/%s\" failed: error %d: %s",
                   str_parent_topic_.c_str(), method.c_str(),
                   response_message.GetErrorCode(),
                   static_cast<std::string>(json_result["message"]).c_str());
      else
        log.ErrorF("Server method \"%s/%s\" failed: error %d",
                   str_parent_topic_.c_str(), method.c_str(),
                   response_message.GetErrorCode());
    ...
    if (pjson_result_out != nullptr)
      *pjson_result_out = response_message.GetResult();
  }

  return (status);
}

Status World::Impl::RequestWorldBool(const std::string& method,
                                     const json& json_params,
                                     bool* pf_result_out) const {
  json result;
  Status status = RequestWorldJson(method, json_params, &result);

  if (status == Status::OK && pf_result_out != nullptr)
    *pf_result_out = result.is_boolean() && static_cast<bool>(result);
...
Status World::Impl::ImportNEDTrajectory(
    const std::string& traj_name, const std::vector<float>& time,
    const std::vector<float>& pose_x, const std::vector<float>& pose_y,
    const std::vector<float>& pose_z, const std::vector<float>& pose_roll,
    const std::vector<float>& pose_pitch, const std::vector<float>& pose_yaw,
    const std::vector<float>& vel_lin_x, const std::vector<float>& vel_lin_y,
    const std::vector<float>& vel_lin_z) {
  const size_t point_count = time.size();
  if (point_count == 0 || pose_x.size() != point_count ||
      pose_y.size() != point_count || pose_z.size() != point_count) {
    return Status::Failed;
  }

  auto make_zero_vector = [point_count]() {
    return std::vector<float>(point_count, 0.0f);
  };

  std::vector<float> roll = pose_roll.empty() ? make_zero_vector() : pose_roll;
  std::vector<float> pitch = pose_pitch;
  std::vector<float> yaw = pose_yaw;
  std::vector<float> vel_x = vel_lin_x.empty() ? make_zero_vector() : vel_lin_x;
  std::vector<float> vel_y = vel_lin_y.empty() ? make_zero_vector() : vel_lin_y;
  std::vector<float> vel_z = vel_lin_z.empty() ? make_zero_vector() : vel_lin_z;

  if (pitch.empty()) {
    pitch.reserve(point_count);
    for (size_t idx = 1; idx < point_count; ++idx) {
      const float dx = pose_x[idx - 1] - pose_x[idx];
      const float dy = pose_y[idx - 1] - pose_y[idx];
      const float dz = pose_z[idx - 1] - pose_z[idx];
      pitch.emplace_back(std::atan2(-dz, std::hypot(dx, dy)));
    }
    pitch.emplace_back(pitch.empty() ? 0.0f : pitch.back());
  }

  if (yaw.empty()) {
    yaw.reserve(point_count);
    for (size_t idx = 1; idx < point_count; ++idx) {
      const float dx = pose_x[idx - 1] - pose_x[idx];
      const float dy = pose_y[idx - 1] - pose_y[idx];
      yaw.emplace_back(std::atan2(dy, dx));
    }
    yaw.emplace_back(yaw.empty() ? 0.0f : yaw.back());
  }

  if (roll.size() != point_count || pitch.size() != point_count ||
      yaw.size() != point_count || vel_x.size() != point_count ||
      vel_y.size() != point_count || vel_z.size() != point_count) {
    return Status::Failed;
  }

  const json json_params = {
      {"traj_name", traj_name}, {"time", time},         {"pose_x", pose_x},
      {"pose_y", pose_y},       {"pose_z", pose_z},     {"pose_roll", roll},
      {"pose_pitch", pitch},    {"pose_yaw", yaw},      {"vel_x_lin", vel_x},
      {"vel_y_lin", vel_y},     {"vel_z_lin", vel_z},
  };

  Message message;
  Status status =
      pclient_->Request(str_parent_topic_ + "/ImportNEDTrajectory",
                        json_params, &message);
  ...
  if (responsemessage.GetErrorCode() != 0 ||
      !static_cast<bool>(responsemessage.GetResult())) {
    return Status::RejectedByServer;
  }
