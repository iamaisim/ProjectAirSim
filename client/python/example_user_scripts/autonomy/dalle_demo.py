...
MIT License.
ProjectAirSim:: Autonomy building-blocks::DALL-E Demo for Real-time Data Augmentation
This Demo App detects landing pads in the scene and augments the scene with a
DALL-E generated image of the landing pad.
"""
import asyncio
import time
import commentjson
        ...
        for i, move in enumerate(moves):
            ...
            if "time-of-day" in move:
                world.set_time_of_day(
                    True, move.get("time-of-day"), False, 1.0, 1.0, True
                )

            name = move.get("name")
            projectairsim_log().info(f"**** {name} Started ****")

            for step_num in range(move["num-loops"]):
                ...
                rgb_reshaped = np.array(rgb["data"]).reshape(
                    (rgb["width"], rgb["height"], 3)
                )
                rgb_reordered = rgb_reshaped[:, :, ::-1].copy()
                # Detect the landing pad in the image
                prediction = predict(rgb)
                # Create a mask for the landing pad
                mask = np.zeros((rgb["width"], rgb["height"]), dtype=np.uint8)
                cv2.rectangle(
                    mask,
                    (int(prediction[0]), int(prediction[1])),
                    (
                        int(prediction[0] + prediction[2]),
                        int(prediction[1] + prediction[3]),
                    ),
                    255,
                ...
                cv2.imwrite(
                    (os.path.join("images", f"rgb_{step_num}.png")), rgb_reordered
                )
                cv2.imwrite((os.path.join("images", f"mask_{step_num}.png")), mask)
                # Create output directory if it doesn't exist
                if not os.path.exists("output"):
                    os.makedirs("output")
                # Use the DallE Edit API
                DalleImageGenerator.edit(
                    prompt="Apply Sunlight Filter",
                    image_path=f"images/rgb_{step_num}.png",
                    mask_path=f"images/mask_{step_num}.png",
                    num=1,
                    size="1024x1024",
                    output_dir=f"./output/{step_num}/",
                )
