...
async def main():
    ...
    try:
        ...
        world = World(client, "scene_two_drones_sensors.jsonc", delay_after_load_sec=2, actual_load=False)
        ...
        # Command the Drone to move southwest in NED coordinate system for 60 seconds
        move_horizontal_task = await drone.move_by_velocity_async(
            v_north=-10.0, v_east=-5.0, v_down=0.0, duration=60.0
        )  # schedule an async task to start the command
        projectairsim_log().info("Move-Horizontal invoked")

        # Example 2: Wait for move_horizontal_task to complete before continuing
        while not move_horizontal_task.done():
            await asyncio.sleep(0.005)
        projectairsim_log().info("Move-Horizontal completed")
        ...
        # Example 3: Wait for move_down_task to complete before continuing
