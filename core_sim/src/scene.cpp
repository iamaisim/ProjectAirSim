...
const std::vector<std::reference_wrapper<Actor>>& Scene::GetEnvActors() const {
...
const std::vector<std::reference_wrapper<Actor>>& Scene::Impl::GetEnvActors()
    const {
    ...
    try {
      ...
      for (auto& env_actor : env_actors_) {
        if (env_actor->GetType() == ActorType::kEnvActor) {
          ...
          env_actor_temp.UpdateKinematics(curr_time);
        }
      }
    } catch (const Error& e) {
...
std::unique_ptr<Actor> Scene::Loader::LoadEnvActorWithJSON(const json& json) {
  auto id = GetActorID(json);
  auto type = GetActorType(json, id);
  auto origin = GetActorOrigin(json, id);
  ...
  if (type == Constant::Config::env_actor) {
    ...
    return std::unique_ptr<Actor>(env_actor);
