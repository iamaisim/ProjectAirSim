...
class TopicManager::Impl {
 ...
 private:
  ...
  bool Unsubscribe(const std::string& topic_path);
  void UnsubscribeAll();
  void Send(const Topic& topic, const Message& message);
  void Send(const std::shared_ptr<TopicBlock>& block);
...
void TopicManager::Impl::Start() {
  ...
  if (rv != 0) {
    ...
    log_.LogError(Constant::Component::topic_manager,
                  "nng_pair0_open failed with '%s'.", errno_str);
...
void TopicManager::Impl::RegisterTopic(
  ...
  log_.LogVerbose(name_, "Registered topic %s", topic.GetPath().c_str());
}
...
void TopicManager::Impl::SubscribeTopic(
  ...
  log_.LogVerbose(name_, "Successfully subscribed to topic '%s'.",
                  topic.GetPath().c_str());
}
...
void TopicManager::Impl::UnregisterTopic(const Topic& topic) {
  ...
  log_.LogVerbose(name_, "Unregistered topic %s", topic.GetPath().c_str());
}
...
void TopicManager::Impl::RecvLoop() {
  while (state_.load()) {
...

    {
      ...
      if (iter == topic_table_.end()) {
        log_.LogError(
            ...
            topic_path.c_str());
        nng_free(recv_buffer_, recv_buffer_size_);
        continue;
      }
    ...
    if (frame.type == FrameType::kSubscribe) {
      send_dispatcher_.dispatch([this, topic]() { Subscribe(topic); });
    } else if (frame.type == FrameType::kUnsubscribe) {
      send_dispatcher_.dispatch([this, topic]() { Unsubscribe(topic); });
...
void TopicManager::Impl::Subscribe(const Topic& topic) {
  std::shared_lock<std::shared_timed_mutex> shared_lock(manager_lock_);
  ...
  log_.LogTrace(name_, "Client subscribing to topic %s", topic_path.c_str());
...
bool TopicManager::Impl::Unsubscribe(
    const std::vector<std::string>& topic_paths) {
  auto success = true;
...
void TopicManager::Impl::Unsubscribe(const Topic& topic) {
  auto topic_path = topic.GetPath();
  Unsubscribe(topic_path);
}

void TopicManager::Impl::UnsubscribeAll() {
  std::shared_lock<std::shared_timed_mutex> shared_lock(manager_lock_);
