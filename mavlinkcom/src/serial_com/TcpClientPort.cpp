...
class TcpClientPort::TcpSocketImpl {
 ...
 public:
  ...
  static void resolveAddress(const std::string& ipAddress, int port,
    ...
    int rc =
        getaddrinfo(ipAddress.c_str(), serviceName.c_str(), &hints, &result);
    if (rc != 0) {
      auto msg = Utils::stringf(
          "TcpClientPort getaddrinfo failed with error: %d\n", rc);
    ...
    if (!found) {
      auto msg = Utils::stringf(
          "TcpClientPort could not resolve ip address for '%s:%d'\n",
  ...
  int connect(const std::string& localHost, int localPort,
    ...
    if (rc < 0) {
      ...
      auto msg = Utils::stringf(
          "TcpClientPort socket bind failed with error: %d\n", hr);
      throw std::runtime_error(msg);
    }
    ...
    if (rc != 0) {
      int hr = GetSocketError();
      auto msg = Utils::stringf(
          "TcpClientPort socket connect failed with error: %d\n", hr);
      throw std::runtime_error(msg);
    }
  ...
  void accept(const std::string& localHost, int localPort) {
    ...
    socklen_t addrlen = sizeof(sockaddr_in);
    int rc =
        ::bind(accept_sock, reinterpret_cast<sockaddr*>(&localaddr), addrlen);
    if (rc < 0) {
      int hr = GetSocketError();
      auto msg = Utils::stringf(
          "TcpClientPort socket bind failed with error: %d\n", hr);
    ...
    if (rc < 0) {
      int hr = GetSocketError();
      auto msg = Utils::stringf(
          "TcpClientPort socket listen failed with error: %d\n", hr);
      throw std::runtime_error(msg);
    }

    // accept 1
    sock = ::accept(accept_sock, reinterpret_cast<sockaddr*>(&remoteaddr),
                    &addrlen);
    if (sock == INVALID_SOCKET) {
      int hr = GetSocketError();
      auto msg =
          Utils::stringf("TcpClientPort accept failed with error: %d\n", hr);
      throw std::runtime_error(msg);
    }
  ...
  void setNonBlocking() {
    ...
    if (rc != 0) {
      rc = GetSocketError();
      auto msg = Utils::stringf(
          "TcpClientPort setNonBlocking failed with error: %d\n", rc);
      throw std::runtime_error(msg);
    }
  }
  ...
  void setNoDelay() {
    ...
    int rc = ::setsockopt(sock, IPPROTO_TCP, TCP_NODELAY,
                          reinterpret_cast<char*>(&flags), sizeof(flags));
#else
    int fd = static_cast<int>(sock);
    ...
    if (rc != 0) {
      rc = GetSocketError();
      auto msg =
          Utils::stringf("TcpClientPort set TCP_NODELAY failed: %d\n", rc);
      throw std::runtime_error(msg);
    }
  }
  ...
  int write(const uint8_t* ptr, int count) {
    ...
    if (hr == SOCKET_ERROR) {
      hr = checkerror();
      auto msg = Utils::stringf(
          "TcpClientPort socket send failed with error: %d\n", hr);
      throw std::runtime_error(msg);
    }
...
TcpClientPort::TcpClientPort() { impl_.reset(new TcpSocketImpl()); }

TcpClientPort::~TcpClientPort() { close(); }

void TcpClientPort::close() { impl_->close(); }

void TcpClientPort::connect(const std::string& localHost, int localPort,
                            const std::string& remoteHost, int remotePort) {
...
bool TcpClientPort::isClosed() { return impl_->isClosed(); }

std::string TcpClientPort::remoteAddress() { return impl_->remoteAddress(); }

int TcpClientPort::remotePort() { return impl_->remotePort(); }

int TcpClientPort::getRssi(const char* ifaceName) {
  return impl_->getRssi(ifaceName);
}

void TcpClientPort::setNoDelay() { impl_->setNoDelay(); }

void TcpClientPort::setNonBlocking() { impl_->setNonBlocking(); }
