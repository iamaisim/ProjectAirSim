...
No jobs run on push, PR open/reopen/update, or manual dispatch. Remove/re-add
...
relevant. A failure blocks subsequent stages. Documentation runs independently
but must pass before optional final regression.
...
Python matrix/result, initial Linux, Windows, macOS, documentation, final Linux,
...
```text
python -m pytest -q test_json_schema.py test_datacollection/test_datacollector_config.py
```
