...
The native sequence is Linux SimLibs on self-hosted Ubuntu 22.04, then requested Linux integration in Blocks Editor, then selected Windows,
...
regression runs after the initial Linux build and relevant Config/Python checks succeed. Windows and macOS wait for regression to pass; without run-regressions, they proceed after the initial Linux build.
...
Python matrix/result, initial Linux, Linux integration, Windows, macOS,
...
Final Linux uses a clean checkout and UE 5.8 selected from `UE_ROOT_5_8`. SimLibs unit tests, standalone C++
build/tests and its package-consumer check, and ROS2 build/tests all use Debug.
Editor integration additionally builds SimLibs Release, ShaderCompileWorker and BlocksEditor Development.
Unreal reuses that same job's Release libraries without another SimLibs build.
The simulator runs through UnrealEditor with Blocks.uproject and -game, using uncooked content; no Unreal packaging is performed. Linux uploads no artifacts.
...
python tools/ci/check_unreal_root.py
...
Unreal roots are read from host `UE_ROOT_5_2` (initial Linux) and `UE_ROOT_5_8` (final regression), with bounded interactive
...
2. PAS Runtime runs control, clock/step, pose/kinematics, battery, non-rendered
   sensor, connection, and client CPU checks before building Blocks Editor. Runtime
