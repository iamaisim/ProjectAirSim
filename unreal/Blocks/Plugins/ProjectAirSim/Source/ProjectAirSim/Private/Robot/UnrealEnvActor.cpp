...
void AUnrealEnvActor::Initialize(const projectairsim::EnvActor& InEnvActor) {
  ...
  this->SimEnvActor =
      InEnvActor;  // This makes a copy from the env actor ref. It does
  ...
  // Set the initial pose from the env actor's kinematics
  UpdateEnvActorTargetPose(InEnvActor.GetKinematics().pose, 0);  // timestamp=0
...
void AUnrealEnvActor::MoveEnvActorToTargetPose(bool bUseCollisionSweep) {
  ...
  projectairsim::Pose TgtPose = EnvActorTargetPose;
...
void AUnrealEnvActor::Tick(float DeltaTime) {
  ...
  UpdateEnvActorTargetPose(SimEnvActor.GetKinematics().pose,
                           0);      // timestamp=0
  MoveEnvActorToTargetPose(false);  // move without collision sweep
  RotateEnvActorLinksToAngle();
  RotateEnvActorLinksAtRate();
