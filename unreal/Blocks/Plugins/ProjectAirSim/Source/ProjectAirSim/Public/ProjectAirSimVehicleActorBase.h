 ...
 * parameter response. Override SetParameterSignal in your Blueprint for custom
 * behaviour.
 *
 * Default behaviour:
 ...
 *  - SetParameterSignal() -> stores the indexed value; Tick applies forces
 *                                when
...
class PROJECTAIRSIM_API AProjectAirSimVehicleActorBase
 ...
 public:
  ...
  /** Stores the indexed value for use in Tick. Override in Blueprint for
   *  custom logic. */
  virtual void SetParameterSignal_Implementation(int32 Index, float Value) override;
 ...
 private:
  TArray<float> StoredParameterSignals_;
};
