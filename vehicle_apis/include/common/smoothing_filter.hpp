...
#ifndef MULTIROTOR_API_INCLUDE_COMMON_SMOOTHING_FILTER_HPP_
#define MULTIROTOR_API_INCLUDE_COMMON_SMOOTHING_FILTER_HPP_
...
// SmoothingFilter is a filter that removes the outliers that fall outside a
// given percentage from the min/max range of numbers over a given window size.
template <typename T>
...
SmoothingFilter<T>::SmoothingFilter() {
  initialize(static_cast<int>(buffer_.capacity()),
...
std::tuple<double, double> SmoothingFilter<T>::filter(T value) {
  ...
  if (buffer_index_ == window_size_2x_) buffer_index_ = window_size_;
  ...
  if (buffer_index_ >= window_size_) {
    ...
    if (count == 0) {
      // this can happen if the numbers are oddly clustered around the min/max
      // values.
    ...
    // window is not yet full, so return an error code informing the caller that
    // the filter is not yet ready.
...
#endif  // MULTIROTOR_API_INCLUDE_COMMON_SMOOTHING_FILTER_HPP_
