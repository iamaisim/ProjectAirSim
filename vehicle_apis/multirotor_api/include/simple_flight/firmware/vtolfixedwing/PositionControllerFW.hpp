...
#ifndef MULTIROTOR_API_SIMPLE_FLIGHT_FIRMWARE_POSITIONCONTROLLERFW_HPP_
#define MULTIROTOR_API_SIMPLE_FLIGHT_FIRMWARE_POSITIONCONTROLLERFW_HPP_
...
// interface
class PositionControllerFW : public IFWAngleController,
                             public IGoal  // for internal child controller
{
 public:
  PositionControllerFW(Params* params, const IBoardClock* clock = nullptr)
  ...
  void Initialize(unsigned int axis, const IGoal* goal,
    ...
    velocity_controller_.reset(new VelocityControllerFW(params_, clock_));
  ...
  void Update() override {
    ...
    // final output
    angle_output_ = velocity_controller_->GetOutputAngle();
 ...
 private:
  ...
  std::unique_ptr<VelocityControllerFW> velocity_controller_;
};

}  // namespace simple_flight

#endif  // MULTIROTOR_API_SIMPLE_FLIGHT_FIRMWARE_POSITIONCONTROLLERFW_HPP_